...
/*
    2/26/2020  v4
    Support objects constructed from supplied vertices and normals.
 */
...
/**
 * Shapes described by triangles and, possibly normals.
 * @type {cs4722_Shapes.Triangles}
 */
cs4722_Shapes.Triangles =  class extends cs4722_Shapes.Base {

    /**
     * Parameter is an object with fields vertices, faces, and normals.
     * This object is used to recover the lists of vertices and normals.
     *
     * @param json
     */
    constructor(json) {
        super();
        this.json = json;
        this.scheme_colors = [X11.NavajoWhite, X11.ForestGreen];
        this.flip_normals = false;
    }

    getSize() {
        return this.json.faces.length * 3;
    }

    getVertices() {
        let verts = [];
        for(let face of this.json.faces) {
            for(let i of face) {
                if(i === 0) {
                    console.log("face index is 0");
                }
                let vtx = this.json.vertices[i-1];
    ...
    getNormals() {
        ...
        for(let face of this.json.faces) {
            for(let i of face) {
                let nml = this.json.normals[i-1];
                if(this.flip_normals) {
                    normals.push([-nml[0], -nml[1], -nml[2], 0]);
                } else {
                    normals.push([nml[0], nml[1], nml[2], 0]);
                }
            }
    ...
    getColorScheme() {
        ...
        for(let i = 0; i < size; i += 3 ) {
            let c = this.scheme_colors[i % this.scheme_colors.length];
            colors.push(c);
            colors.push(c);
            colors.push(c);
        }
